...
export function formatUSDT(value: ValueType, round = false, digits = 4) {
  if (round) {
    return bigintToFloat(value, 6).toLocaleString(undefined, {
      maximumFractionDigits: digits,
    })
  }

  const number = bigintToFloat(value, 6)
    .toLocaleString(undefined, {
      maximumFractionDigits: 18,
    })
    .split('.')
